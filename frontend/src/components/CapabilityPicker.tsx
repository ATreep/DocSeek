...
type Capability = { id: string; label: MessageKey }

const groups: Array<{ label: MessageKey; capabilities: Capability[] }> = [
...
export default function CapabilityPicker({ value, onChange }: { value: string[]; onChange: (capabilities: string[]) => void }) {
  ...
  return <div className="capability-picker" aria-label={t('Role capabilities')}>
    <div className="capability-picker-summary"><span>{t('Capabilities')}</span><strong>{t('{count} selected', { count: value.length })}</strong></div>
    {groups.map(group => <fieldset className="capability-group" key={group.label}><legend>{t(group.label)}<button type="button" aria-label={t('{action} {group} capabilities', { action: group.capabilities.every(capability => selected.has(capability.id)) ? t('Clear') : t('Select all'), group: t(group.label) })} onClick={() => toggleGroup(group.capabilities)}>{group.capabilities.every(capability => selected.has(capability.id)) ? t('Clear') : t('Select all')}</button></legend><div className="capability-options">{group.capabilities.map(capability => <label className="capability-option" key={capability.id}><input type="checkbox" aria-label={t(capability.label)} checked={selected.has(capability.id)} onChange={() => toggle(capability.id)} /><span>{t(capability.label)}<small>{capability.id}</small></span></label>)}</div></fieldset>)}
  </div>
}
