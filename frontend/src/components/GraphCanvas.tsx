...
export default function GraphCanvas({ graph, kind, onSelect, onRelationSelect }: GraphCanvasProps) {
  ...
  return <section className="graph-panel">
    <div className="panel-toolbar"><div><strong>{kind === 'property' ? t('Property Graph') : t('Entity Graph')}</strong><span className="count-label">{t('{shown} of {total} nodes \u00b7 {edges} relations', { shown: nodes.length, total: graph.nodes.length, edges: graph.edges.length })}</span></div><div className="icon-actions">{searchOpen && <input aria-label={t('Graph filter')} placeholder={t('Filter graph nodes')} value={filter} onChange={event => setFilter(event.target.value)} autoFocus />}<button type="button" aria-label={t('Search graph')} title={t('Search graph')} onClick={() => setSearchOpen(open => !open)}><Search size={15} /></button><button type="button" aria-label={t('Zoom out')} title={t('Zoom out')} onClick={() => adjustZoom(-0.2)}><ZoomOut size={15} /></button><button type="button" aria-label={t('Zoom in')} title={t('Zoom in')} onClick={() => adjustZoom(0.2)}><ZoomIn size={15} /></button><button type="button" aria-label={t('Fit all')} title={t('Fit all')} onClick={fitAll}><Maximize2 size={15} /></button><button type="button" aria-label={t('Focus selection')} title={t('Focus selection')} onClick={focusFirstNode}><Focus size={15} /></button><button type="button" aria-label={t('Reset layout')} title={t('Reset layout')} onClick={resetLayout}><RefreshCcw size={15} /></button></div></div>
    {graph.nodes.length ? <div className="graph-body"><nav className="graph-index" aria-label={kind === 'property' ? t('Property nodes') : t('Entity nodes')}><div className="graph-index-heading">{kind === 'property' ? t('Properties') : t('Entities')}<span>{graph.nodes.length}</span></div><ul className="graph-index-list">{graph.nodes.map(node => <li key={node.id}><button type="button" aria-label={graphNodeLabel(node)} className={`graph-list-item ${kind} ${selectedNodeId === node.id ? 'selected' : ''}`} onClick={() => selectNode(node)}><CircleDot size={14} /><span>{graphNodeLabel(node)}</span><small>{node.property_type || node.type || kind}</small></button></li>)}</ul></nav><div className="graph-visual"><div ref={canvasRef} className="graph-canvas" role="application" aria-label={kind === 'property' ? t('Interactive property node-edge graph') : t('Interactive entity node-edge graph')} />{!nodes.length && <div className="graph-filter-empty"><CircleDot size={18} /> {t('No matching nodes')}</div>}</div></div> : <div className="empty-state"><CircleDot size={28} /><strong>{t('Your graph is waiting')}</strong><span>{t('Upload a property to create the first candidate snapshot.')}</span></div>}
  </section>
}
