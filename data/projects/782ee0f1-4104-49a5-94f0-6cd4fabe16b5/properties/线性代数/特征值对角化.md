...
> **常数函数的「函数点积」**
> 
> 我们可以注意到，傅里叶级数对 $f(x)$ 的展开的第一项是一个常数项 $a_{0}$。
> 如果我们希望通过函数 “正交” 方式得到 $a_{0}$，类比上述做法可以得到：
> $$\int^{2\pi}_{0}1\cdot1dx=2\pi$$
> 为了使结果为 1，当计算 $a_{0}$ 时，应该除以 $2\pi$ 以归一化，而不仅仅是除以 $\pi$。
> 或者，我们也可以在傅里叶级数的展开式中，将原本的 $a_{0}$ 项表示为 $\frac{a_{0}}{2}$。
> 
> 可得 $f(x)$ 以 $2\pi$ 为周期的傅里叶级数为：$f(x)\sim\frac{a_{0}}{2}+\sum_{n=1}^\infty(a_{n}\cos(nx)+b_{n}\sin(nx))$
